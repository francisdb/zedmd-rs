//! RGB888 plasma animation example.
//!
//! Submits frames as RGB888 but converts to RGB565 before sending — matching
//! libzedmd's default `RenderRgb888` behaviour (`m_rgb888 = false`). This is
//! the only path verified to work: libzedmd's own test suite never calls
//! `EnableTrueRgb888`, and command `0x04` (RGB888ZonesStream) fails even in
//! the reference C++ library. See <https://github.com/PPUC/libzedmd/issues/47>.
//!
//! Run with:
//! ```sh
//! cargo run --example plasma_rgb888                    # USB
//! cargo run --example plasma_rgb888 -- --wifi          # ZeDMD-WiFi.local
//! cargo run --example plasma_rgb888 -- --wifi 10.0.1.7 # explicit host
//! ```
use log::{error, info};
use std::io;
...
fn run() -> io::Result<()> {
    ...
    info!(
        "Connected: {}x{}, refresh rate cap: {} fps (RGB888 → RGB565)",
